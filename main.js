...
// Đợi tới khi nút Gửi THỰC SỰ sẵn sàng (không bị khóa) — dùng đúng icon mũi tên lên
// đã xác định trước đó (data-mat-icon-name="arrow_upward"). Nhiều khung chat khóa nút
// Gửi trong lúc tệp đính kèm đang tải lên ngầm phía sau, chỉ mở khóa khi tải xong hẳn —
// đây là tín hiệu đáng tin cậy hơn nhiều so với chỉ thấy ảnh preview hiện ra.
async function waitForSendButtonReady(wc, timeoutMs) {
  ...
  try {
    ...
    if (refImageBase64) {
      ...
      try {
        ...
        if (plusClicked) {
          ...
          if (uploadClicked) {
            ...
            // Kiểm tra CHỦ ĐỘNG (tối đa 8s) xem ảnh preview đã thực sự hiện trong ô soạn
            // tin chưa — thay vì chỉ chờ cố định 1.5s (dễ gửi thiếu ảnh nếu mạng/máy chậm)
            const attachDeadline = Date.now() + 8000;
            let attachConfirmed = false;
            while (Date.now() < attachDeadline) {
              await sleepMs(400);
              const hasPreview = await wc.executeJavaScript(`
                (function(){
                  const input = document.querySelector('[contenteditable="true"]') || document.querySelector('textarea');
                  ...
                  for(let d=0; d<6 && container; d++, container=container.parentElement){
                    ...
                    for(const im of imgs){
                      if(im.naturalWidth>10 && im.naturalWidth<400) return true; // ảnh preview nhỏ trong ô soạn tin
                    }
                  }
                  return false;
                })();
              `).catch(() => false);
              if (hasPreview) { attachConfirmed = true; break; }
            }
            if (!attachConfirmed) {
              throw new Error('Đính kèm ảnh tham chiếu thất bại — không thấy ảnh preview trong ô soạn tin sau 8 giây.');
            }
            // Ảnh preview đã hiện, nhưng file THẬT có thể vẫn đang tải lên ngầm phía sau —
            // đợi thêm tới khi nút Gửi thực sự sẵn sàng (không bị khóa) mới coi là xong hẳn
            const uploadReady = await waitForSendButtonReady(wc, 15000);
            if (!uploadReady) {
              throw new Error('Ảnh đã hiện preview nhưng nút Gửi vẫn bị khóa sau 15s — có thể vẫn đang tải lên.');
            }
            await sleepMs(800); // khoảng đệm an toàn, đảm bảo giao diện ổn định hoàn toàn trước khi gửi
      ...
      if (!attachedViaMenu) {
        ...
        // Kiểm tra CHỦ ĐỘNG (tối đa 8s) xem ảnh preview đã dán thành công chưa
        const pasteDeadline = Date.now() + 8000;
          ...
          const hasPreview = await wc.executeJavaScript(`
            (function(){
              ...
              for(let d=0; d<6 && container; d++, container=container.parentElement){
                const imgs = container.querySelectorAll('img');
                for(const im of imgs){
                  if(im.naturalWidth>10 && im.naturalWidth<400) return true;
                }
              }
              return false;
            })();
          `).catch(() => false);
          if (hasPreview) { pasteConfirmed = true; break; }
        }
        if (!pasteConfirmed) {
          console.warn('[Gemini Web] Không xác nhận được ảnh preview sau khi dán clipboard — vẫn tiếp tục nhưng có thể ảnh chưa gắn kịp.');
        }
        if (pasteConfirmed) {
          // Đợi thêm tới khi nút Gửi thực sự sẵn sàng (tệp có thể vẫn đang tải lên ngầm)
          await waitForSendButtonReady(wc, 15000);
        }
        await sleepMs(800); // khoảng đệm an toàn, đảm bảo giao diện ổn định hoàn toàn trước khi gửi
